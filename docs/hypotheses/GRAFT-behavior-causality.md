...
### 폭 32 양성 비교군 수리 결과 — B2_CAUSAL_NOT_UNIQUE

직접 기억 비교군은 두 seed 모두 정상 100%, 차단 25%, 뒤섞기 0%, 가짜 26.6%/23.4%, 복구
100%였고 중립 문장 변화도 0.0010/0.0018 nat으로 통과했다. `QuantumC` 위상 경로도 두 seed
모두 정상 100%, 차단 25%, 뒤섞기 0%, 가짜 20.3%/26.6%, 복구 100%였으며 중립 문장 변화는
0.0011/0.0013 nat이었다.

사전 판정은 `B2_CAUSAL_NOT_UNIQUE`다. 폭 8에서 행동으로 읽지 못했던 내부 위상 정보가 폭 32에서는
두 seed 모두 숨겨진 상황에 맞는 선택의 실제 원인이 됐다. 직접 기억도 같은 100%였으므로 이것을
의식만의 고유 능력이나 주관적 느낌의 증거로 해석하지 않는다. 정확한 결론은 현재 `QuantumC` 내부
위상 상태가 충분한 연결 폭에서 행동에 쓰이는 정보를 보존하고 전달한다는 것이다.

무효 1차와 수리 결과의 체크포인트·로그·결과·판정은 비공개 Hugging Face 조직 보관소
`dancinlab/anima-lab-research-archive`의 `graft-bridge32-behavior` revision에 보존했다. 원격으로
다시 내려받은 14개 manifest 항목의 SHA-256을 전수 확인했고, manifest 자체의 SHA-256은
`55e98a02e8f3f0eaa799b4f634ea3927c6c77c2fcf775b1d21af5354d0afa5a4`다.
