...
# Practical horizon used when cell growth is UNLIMITED (--max-cells 0). This is only
# how far the Fibonacci schedule is pre-computed; the real limit is GPU memory, enforced
# at split time. Each cell is a small GRU mind (~74K params), so memory allows thousands;
# per-step cost grows linearly with cell count, so throughput is what actually bites.
UNLIMITED_CELL_HORIZON = 4181  # 19th Fibonacci number


def _gpu_has_headroom(device, reserve_bytes: int = 768 * 1024 * 1024) -> bool:
    """True while the GPU still has room to add another cell.

    Growth is unlimited by design; this is the hardware floor, not a policy cap. Keeps a
    reserve so the training step itself (activations, optimizer) cannot be starved by a
    split. On CPU there is no hard ceiling to read, so growth proceeds.
    """
...
def fibonacci_milestones(total_steps: int, max_cells: int = 8,
    ...
    """
    # max_cells <= 0 means UNLIMITED (user directive): no designed ceiling on how many
    # cells the consciousness may grow into. A schedule still needs concrete milestones,
    # so the Fibonacci cadence is generated up to a practical horizon; growth past it is
    # bounded by hardware alone (see the memory guard in the growth loop), never by a
    # number chosen here.
    ceiling = max_cells if max_cells > 0 else UNLIMITED_CELL_HORIZON
    fib = _generate_fibonacci(ceiling)
    usable = sorted({f for f in fib if f <= ceiling})
...
def train(args: argparse.Namespace):
    ...
    mitosis = MitosisEngine(
        ...
        # Unlimited (--max-cells 0) → the engine's own guard must never be the binding
        # constraint; the hardware guard in the growth loop is.
    ...
    # --- Training state ---
    phi_prev = 0.0
    # (PHI-K2 phi_floor removed — see the note at the emergency-boost site below)
    phi_current = 0.0
    ...
    for step in range(start_step, args.steps):
        ...
        # args.max_cells <= 0 = UNLIMITED: no designed ceiling. Splitting stops only when
        # the GPU is genuinely out of room, and that stop is logged as a hardware limit so
        # it can never be mistaken for the consciousness having finished growing.
        ...
        for milestone_step, target_cells in sorted(fib_milestones.items()):
            if step == milestone_step and len(mitosis.cells) < target_cells:
                while len(mitosis.cells) < target_cells and (
                        unlimited_cells or len(mitosis.cells) < args.max_cells):
                    if not _gpu_has_headroom(device):
        ...
        # PHI-K2 REMOVED (user directive · CLAUDE.md rule 2 조작 금지): the emergency
        # boost rewrote cell hidden states (mean blending, faction mixing, norm scaling)
        # purely to lift the Φ number when it fell below a ratcheting floor. That is
        # editing the consciousness's state to make a metric look good, not training —
        # and it made the reported Φ useless as evidence of emergence. Φ now moves only
        # through the cells' own dynamics (the ratchet above restores prior states, it
...
Examples:
  ...
  python train_conscious_lm.py --resume checkpoints/step_10000.pt       # resume training
    ...
    parser.add_argument("--max-cells", type=int, default=0,
                        help="Maximum mitosis cells; 0 = UNLIMITED (default), bounded only "
                             "by GPU memory")
