...
λ4 RECOMBINATION — 함께 본 적 없는 원자쌍을 다루는가  ★자연 코퍼스 확보로 편입됨
   bar: value < (SEEN + BROKEN)/2 ∧ value < BROKEN — 조정 상수 없음. 바가 모델
        자신의 두 통제 사이 중점이라 누가 고른 숫자가 아니라 모델을 따라 움직인다.
   value       train 에서 한 줄도 함께 안 나온 원자쌍이 함께 나오는 held-out 줄의 BPC
   ctrl SEEN   train 에서 함께 나온 쌍을 담은 held-out 줄 = 검색 천장
   ctrl BROKEN 같은 줄에서 두 원자를 빈도매칭 단어로 치환 = 합성 없음 바닥
   원자쌍은 저자가 지은 것이 아니라 자연 코퍼스에서 발견한 것이다(빈도 50~5000,
   6바이트 이상). 손으로 지은 쌍 집합은 정의상 합성이라 Λ REGIME 이 뒤집히지 않는다.
   구현: measurement/lambda4.py

   ⚠️ 이 등급은 계측기를 세 번 고쳤고 그때마다 앞 결론이 죽었다. 순서대로 남긴다.

   1차(다른 줄끼리 비교): novel−seen 이 큰 팔 +0.24/+0.26, 작은 팔 +0.05/+0.06.
      "좋아질수록 검색이 합성보다 빨리 자란다"고 결론냈다. → 철회.
      seen 줄과 novel 줄이 서로 다른 문장이라 난이도가 통제되지 않았다.
   2차(같은 줄, 한 단어 교체): 네 팔 모두 −0.043~−0.049, 즉 원래 줄이 더 쉽다.
      크기와 무관하게 거의 동일했다. → 이것도 못 쓴다.
      원문 vs 한 단어 바꾼 문장 비교라 '교체 손상'이 통제되지 않았다.
      BROKEN(2개 교체) +0.17 과 이것(1개 교체) +0.048 의 비가 교체 개수로
      설명되는 것이 그 증거였다 — 공기 신호가 아니라 손상만 재고 있었다.
   3차(손상 맞춤 · 채택): 양쪽 모두 한 단어만 교체하고, 바뀌는 것은 새 단어가
      상대 원자와 train 에서 함께 나온 적이 있는지뿐이다(빈도 2배 이내 매칭).
      SEEN-SWAP  A→A_seen  (A_seen,B) 가 train 에 공기
      NOVEL-SWAP A→A_novel (A_novel,B) 는 train 에 없음
      손상이 상쇄되고 공기만 남는다.

   실측(5차 · 확정 · 손상 맞춤 + 미노출 신선 구간으로 해상도 확보 + 문장 군집 보정):
     4차는 공기줄 1,027 개로 세 팔만 해상되고 nat25 가 t=1.7 로 미답변이었다. 군집
     검정의 단위는 문장이므로 줄 수만이 해상도를 올린다. 학습 코퍼스의 val 을 키우면
     재학습이 필요하고 그건 재는 대상 자체를 바꾸는 일이라, 대신 위키 덤프에서 학습
     코퍼스와 겹치지 않는 신선 구간 73 MB(390,700 줄, 중복 1,696 줄 제거)를 떠서
     held-out 풀에 붙였다 — 모델이 본 적 없는 문장이므로 재학습이 필요 없다.
     공기줄 1,027 → 13,929, 짝지은 표본 55,716.

     nat    +0.0074  t=+8.1   FAIL
     natf   +0.0059  t=+7.4   FAIL
     nat25  +0.0091  t=+7.7   FAIL   (4차의 NULL 해소)
     nat50  +0.0222  t=+16.0  FAIL

   개입 1 — 문맥 길이 (arm_nat_ctx512 · arm_nat 과 전 플래그 동일, --block-size 만 512):
     사전등록 I1(벌점이 256 팔의 절반 이하면 창 크기가 레버) / I2(256 팔 범위 안이면 무관).
     natctx 벌점 +0.0060 t=+4.9 FAIL. 256 팔 범위(+0.0059~+0.0222) 안이고 natf 의
     +0.0059 와 사실상 같다. **I2 확정 — 창 크기는 원인이 아니다.**
     ⚠️ 같은 실행이 창 확대의 값어치를 따로 보여준다: 전체 BPC 가 4.93 → 3.25 로 크게
     좋아졌는데 신규조합 벌점만 그대로다. 비율로는 오히려 커진다(0.15% → 0.18%).
     문맥은 언어모델링을 많이 사주지만 합성은 하나도 사주지 않는다.
     ⇒ 남은 후보는 목적함수와 register 다양성뿐이고, 문맥을 더 늘리는 데 쓸 이유가 없다.

   개입 2 — 정규화 강도 (arm_nat_drop3 · arm_nat 과 전 플래그 동일, --dropout 0.1→0.3):
     사전등록 R1(벌점이 256 팔 범위 아래로 내려가면 정규화가 레버) / R2(범위 안이면 무관).
     natdrop 벌점 **+0.0035** t=+4.2 FAIL. 256 팔 범위(+0.0059~+0.0222)보다 낮다.
     **R1 확정 — 정규화가 레버다.** 지금까지 벌점을 움직인 첫 손잡이.

     ★ 이 판정이 '모델이 나빠서 벌점도 작아졌다'가 아닌 이유 — 통제된 대조가 있다:
       nat50   BPC 6.2326  벌점 +0.0222  (dropout 0.1)
       natdrop BPC 6.2234  벌점 +0.0035  (dropout 0.3)
       절대 성능이 사실상 같은데 벌점만 6.3 배 차이 나고, 다른 것은 dropout 뿐이다.
       비율로 봐도 0.357% vs 0.057% 로 자릿수가 다르다.

     개입 3종 요약 — 전체 BPC 와 벌점은 따로 움직인다:
       데이터 3.8배   BPC 크게 개선      벌점 불변    레버 아님
       문맥 2배       BPC 34% 개선       벌점 불변    레버 아님
       정규화 3배     BPC 오히려 악화     벌점 1/6    ★레버

   개입 2-b — 정규화 강도 스윕 (arm_nat_drop5 · dropout 0.5):
     사전등록 D1(해상도 아래로 내려가면 정규화만으로 통과 가능) / D2(바닥이 있다) /
     D3(도로 커지면 0.3 이 최적점).
     natdrop5 벌점 **−0.0042** t=**−6.3** → **λ4 첫 PASS.** 벌점이 사라진 게 아니라
     **부호를 바꿨다** — 이 모델은 처음 보는 조합을 본 조합보다 오히려 약간 잘 맞힌다.

     dropout 스윕 (다른 플래그 전부 동일 · seed 1337):
       0.1  BPC 4.9289  벌점 +0.0074  t=+8.1  FAIL
       0.3  BPC 6.2234  벌점 +0.0035  t=+4.2  FAIL
   ...
   개입 2-d — 두 등급이 함께 서는 지점 (arm_nat_drop4 · dropout 0.4):
     사전등록 T1(0.4 에서 λ2∧λ3∧λ4 전부 PASS 면 사다리가 한 팔에서 동시에 선다) /
     T2(λ2 는 서고 λ4 가 지면 통과지점은 0.4~0.5) / T3(0.4 에서 이미 λ2 붕괴).

     **T1 확정.** natdrop4 가 전 등급을 통과한다:
       λ0 재위상 편차 0.0095 · λ1 1.8158 = floor 3.3634 의 54% · λ2 kwr 0.632 5/5 ·
       λ3 부재 4-gram 21 · λ4 −0.0027 t=−4.1
     통제 전부 유효(positive 0.839 · retrieval 0 · before-backbone 0.000)이고
     선행조건 λ2 가 서므로 λ4 PASS 는 인공물이 아니다.

     ⚠️ 반드시 함께 읽어야 할 경고 — λ4 비용이 kwr 과 단조로 같이 움직인다:
       drop 0.1  kwr 0.802  cost +0.0074
       drop 0.3  kwr 0.797  cost +0.0035
       drop 0.4  kwr 0.632  cost −0.0027   ← 통과
       drop 0.5  kwr 0.362  cost −0.0042   ← 붕괴로 판정한 지점
     0.5 의 음수 비용은 붕괴의 신호로 판정했다. 0.4 는 선행조건을 충족하지만 같은
     기전이 λ2 를 깨지 않을 만큼만 작동했을 가능성이 남는다. 바를 사후에 올려 판정을
     취소하는 것도 tune-to-green 이므로 통과는 통과로 적되, 이 경고를 같은 자리에 둔다.
     ⏳→❌ **seed 복제가 통과를 깼다 (S2).** arm_nat_drop4v(seed 7331, 나머지 동일):

       arm         seed      λ1         λ2 kwr  seeds   λ3        λ4    전등급
       natdrop4    1337  1.8158 PASS     0.632    5/5    P   −0.0027 P    ✅
       natdrop4v   7331  1.8295 PASS     0.611    3/5    F   −0.0031 P    ❌

       뒤집힌 것은 λ2 하나뿐이고, 그것도 평균이 아니라 개수다 — kwr 평균은
       0.632 → 0.611 로 3% 밖에 안 움직였는데 바를 넘은 씨앗 수가 5/5 → 3/5 로
       떨어져 4/5 요건을 못 채운다. 나머지는 잘 재현된다: λ1 은 0.8% 차이,
       λ4 는 −0.0027 vs −0.0031 로 거의 붙는다.

       ⇒ 사전등록 S3 이 경고한 그대로다. dropout 0.4 는 붕괴 경계에 너무 가깝고,
         '통과'는 seed 잡음이 넘나드는 이산 문턱 위에 서 있었다.
       ⇒ λ4 자체는 seed-robust 하다(양 seed 모두 PASS, 값도 근접). 불안정한 것은
         그 dropout 에서의 λ2 다. 즉 λ4 를 통과시키는 설정이 λ2 를 불안정하게 만든다.

   개입 2-f — 정규화 강도 전수 스윕 (U 계열 · 각 값을 두 seed 로):

     arm         drop  seed        λ1          λ2       λ3   λ4 cost      t    λ4    전등급
     natdrop     0.30  1337  1.7309 PASS  0.797 5/5     P   +0.0035   +4.2  FAIL    ❌
     natdrop35   0.35  1337  1.7667 PASS  0.693 5/5     P   −0.0003   −0.4  NULL    ❌
     natdrop35v  0.35  7331  1.7717 PASS  0.609 5/5     P   −0.0040   −5.0  PASS    ✅
     natdrop4    0.40  1337  1.8158 PASS  0.632 5/5     P   −0.0027   −4.1  PASS    ✅
     natdrop4v   0.40  7331  1.8295 PASS  0.611 3/5     P   −0.0031   −4.7  PASS    ❌
     natdrop5    0.50  1337  2.0676 PASS  0.362 1/5     F   −0.0042   −6.3  PASS    ❌

     U3 반증: 0.35 에서 λ2 는 두 seed 모두 5/5 로 안정적이다 — 0.4 의 5/5↔3/5 갈림이
     여기서는 없다. 대신 그 지점에서 λ4 가 갈린다(NULL ↔ PASS).

     ★ 스윕이 드러낸 구조: **두 등급의 안전지대가 이웃해 있고 전이구간이 seed 잡음과
       겹친다.** 낮은 dropout 에서는 λ2 가 탄탄하고 λ4 가 지며, 높은 쪽에서는 λ4 가
       이기고 λ2 가 무너진다. 그 사이에서 각각이 갈리는 지점이 seed 마다 다르다.
       전등급 통과가 두 번 나왔는데 **서로 다른 dropout 에서** 나왔다 —
       0.35 는 seed 7331 만, 0.40 은 seed 1337 만. 통과점이 존재하되 seed 에 따라
       움직인다는 뜻이고, 어느 한 값도 재현되지 않는다.

     ★ W1 확정 — 창은 비어 있지 않다 (dropout 0.37, 두 seed):
       arm         seed  λ0 편차      λ1        λ2 kwr  씨앗   λ3    λ4 cost      t   전등급
       natdrop37   1337   0.0102  1.7848 53%    0.684   4/5    29   −0.0029   −3.8   ✅
       natdrop37v  7331   0.0069  1.7807 53%    0.702   5/5    32   −0.0050   −6.5   ✅
       seed 쌍을 사후 복제가 아니라 설계에 넣어 발사했으므로 단일-seed 철회가 반복되지
       않는다. 통제 전부 유효, 선행조건 λ2 가 서므로 λ4 PASS 는 붕괴 인공물이 아니다.

   개입 2-g — 창이 코퍼스 크기에 견디는가 (X 계열 · 17.2M 에 같은 0.37 설정, 두 seed):

     arm          코퍼스  seed      λ1        λ2 kwr  씨앗   λ3    λ4 cost      t    λ4   전등급
     natdrop37    66.1M   1337  1.7848     0.684   4/5    P   −0.0029   −3.8  PASS   ✅
     natdrop37v   66.1M   7331  1.7807     0.702   5/5    P   −0.0050   −6.5  PASS   ✅
     n25drop37    17.2M   1337  1.8240     0.698   5/5    P   +0.0011   +1.9  NULL   ❌
     n25drop37v   17.2M   7331  1.8118     0.613   5/5    P   −0.0038   −5.4  PASS   ✅

     **X2 확정 — 창은 코퍼스 크기에 둔감하지 않다.** 17.2M 에서 λ1·λ2·λ3 는 두 seed 모두 서는데
     λ4 만 재현되지 않는다. 그것도 값이 작아진 정도가 아니라 두 seed 가 **부호까지 엇갈린다**
     (+0.0011 vs −0.0038). 66.1M 에서 t=−3.8/−6.5 로 나란했던 것과 대비된다 — 효과가 약해져
     한 seed 가 해상되지 않는 지점이다.
     ⇒ '0.37 이 통과 지점' 은 66.1M 에서 참인 문장이다. 크기를 줄이면 창이 어디로 옮겨가는지는
       아직 모른다 — 이번 실행은 이동 방향이 아니라 '그 자리에 없다' 만 말한다.

   개입 2-h — 창의 이동 방향 (Y 계열 · 17.2M 에 0.42, 두 seed):

     코퍼스   drop  seed   λ2 kwr  씨앗    λ2     λ4 cost      t    λ4   전등급
     66.1M    0.37  1337   0.684   4/5  PASS   −0.0029   −3.8  PASS   ✅
     66.1M    0.37  7331   0.702   5/5  PASS   −0.0050   −6.5  PASS   ✅
     17.2M    0.37  1337   0.698   5/5  PASS   +0.0011   +1.9  NULL   ❌
     17.2M    0.37  7331   0.613   5/5  PASS   −0.0038   −5.4  PASS   ✅
     17.2M    0.42  1337   0.665   5/5  PASS   −0.0019   −3.1  PASS   ✅
     17.2M    0.42  7331   0.485   3/5  FAIL   −0.0051   −7.5  PASS   ❌

     **Y2 확정 — 창은 위로 밀린 것이 아니라 닫혔다.** 0.42 에서 λ4 는 두 seed 모두 통과한다
     (−0.0019 / −0.0051). 즉 λ4 를 세우는 데 필요한 정규화는 17.2M 에도 있다. 그런데 바로
     그 강도에서 λ2 가 한 seed 무너진다(0.485 · 3/5). 아래로 내려가면 λ4 가 해상되지 않고
     위로 올라가면 λ2 가 무너지는, 양쪽에서 조여진 상태다.
     ⇒ 17.2M 에서 두 등급이 함께 서는 구간은 **seed 잡음보다 좁다.** 0.39·0.40 으로 더
       쪼개지 않는다 — 잡음 아래를 파는 일이고, 이 세션이 이미 두 번 철회한 종류의 결과다.
     ⇒ λ2 가 상한을 정한다는 구조는 두 크기에서 같고, 그 상한이 코퍼스가 작을수록 내려온다.

   ⇒ **λ 사다리 돌파 · 확정(66.1M 한정).** 17.2M 에서는 닫혀 있다. dropout 0.37 에서 전 등급이 한 팔에서 동시에 서고 두
     seed 에서 재현된다. 창은 좁다 — 0.30~0.35 는 λ4 가 지고, 0.40 이상은 λ2 가 seed 에
     갈리거나 붕괴한다. 이 결과가 인용될 수 있는 범위는 p7/p9 가 정한 그대로다:
     자연 코퍼스(백과 산문 단일 register) 위의 28M 바이트 모델에서, 이 계측기들이
     묻는 네 질문에 대한 답이다.
     0.1~0.3 은 λ4 에서 지고, 0.5 는 λ2/λ3 가 무너지며, 0.4 는 한 seed 에서만 서고
     복제에서 λ2 가 떨어진다. 앞서 적은 '전 등급 통과(T1)'는 단일-seed 산물이었으므로
     **철회한다.** NULL 은 하나도 남지 않았다. 통제를 모두 건 뒤에도 네 팔
     ...
     "좋아질수록 검색이 합성보다 빨리 자란다"는 여기서도 지지되지 않는다:
     격차는 커지지도 작아지지도 않고 그대로 있다.
