...
λ4 RECOMBINATION — 함께 본 적 없는 원자쌍을 다루는가  ★자연 코퍼스 확보로 편입됨
   ...
   개입 2-d — 두 등급이 함께 서는 지점 (arm_nat_drop4 · dropout 0.4):
     ...
     **T1 확정.** natdrop4 가 전 등급을 통과한다:
     ...
     ⏳ 단일 seed 결과다. seed 7331 복제(arm_nat_drop4v)가 돌고 있으며, S1(네 등급
       전부 재현) 이 확인되기 전까지 '사다리 통과'는 **잠정**이다 — DATA-5 §5 가
       단일-seed 수치를 sampler-walk 착시로 판정한 전례가 이 리포에 이미 있다.

   ⇒ **λ4: 잠정 통과 (seed 복제 대기).** dropout 0.4 에서 전 등급이 동시에 선다.
     0.1~0.3 은 λ4 에서 지고, 0.5 는 λ2/λ3 가 무너진다 — 통과 구간은 좁다. NULL 은 하나도 남지 않았다. 통제를 모두 건 뒤에도 네 팔
