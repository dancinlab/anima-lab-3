...
λ4 RECOMBINATION — 함께 본 적 없는 원자쌍을 다루는가  ★자연 코퍼스 확보로 편입됨
   ...
   개입 2-f — 정규화 강도 전수 스윕 (U 계열 · 각 값을 두 seed 로):
     ...
     ⏳ 창이 정말 비었는지 0.37 두 seed 로 확인 중(W1 비어있지 않음 / W2 폭이 seed
       이동폭보다 좁음=재현 가능한 통과점 없음 / W3 창 밖). W2 가 나오면 0.36·0.38 로
       더 쪼개지 않는다 — 잡음 아래를 파는 일이기 때문이다.

   ⇒ **λ 사다리 미도달 · 확정.** 전 등급을 재현 가능하게 통과하는 팔이 없다.
