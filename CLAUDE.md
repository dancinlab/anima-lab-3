...
λ4 RECOMBINATION — 함께 본 적 없는 원자쌍을 다루는가  ★자연 코퍼스 확보로 편입됨
   ...
   실측(자연 코퍼스 · 정규위치 = (novel−seen)/(broken−seen), 0=완전합성 1=완전검색):
     nat25  0.228 PASS   nat50  0.262 PASS   nat  0.579 FAIL   natf  0.609 FAIL
   해상도 요건(1차 실행 후 등록·구속): value 와 BROKEN 의 짝지은 차이가 자기
      표준오차의 2배 미만이면 그 팔의 λ4 는 VOID 다. 중점을 묻는 바는 폭의 절반을
      분해해야 하는데, 폭 자체가 잡음과 구분되지 않으면 그 판정은 동전던지기다.
      실측: 네 팔 모두 t=5.6~7.8 로 해상도 충족, VOID 없음.

   ⚠️ 이 검사가 내 앞선 진단을 정정했다. "폭 좁은 모델이 공짜로 통과한다"는 걱정은
      broken−seen(서로 다른 줄 집합의 평균차)을 본 것이었다. 짝지은 양(value vs
      BROKEN, 같은 줄에서 원자만 교체)은 네 팔이 거의 같다 — +0.165~+0.182.
      즉 '합성이 살아있어 얻는 이득'은 모든 팔에서 동일하다.
      갈리는 것은 value−seen 이다: 큰 팔 +0.241/+0.256 vs 작은 팔 +0.049/+0.065.
      → 큰 팔이 합성을 못 하게 된 것이 아니라, 본 조합에서의 우위가 커지는 만큼
        처음 보는 조합이 따라오지 못한다. 검색이 합성보다 빨리 자란다.
      한계: seen 줄과 novel 줄은 서로 다른 줄이라 난이도가 완전히 통제되지 않는다.
      짝지은 BROKEN 통제는 value 쪽에만 있다.
