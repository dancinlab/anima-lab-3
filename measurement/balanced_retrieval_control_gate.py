#!/usr/bin/env python3
"""Fail-closed adjudication for GATE-RETRIEVAL-CONTROL-3."""
...
def adjudicate(
    ...
    expected_kind_side = expected_kind // 2
    ...
    for row in seeds:
        ...
        expected_balance = {
            kind: {"0": expected_kind_side, "1": expected_kind_side}
        ...
        if (
            fit.get("method") != "canonical_ridge"
            or fit.get("examples") != REALISTIC_MEMORY_WRITE_SPEC["calibration_rows"]
            or fit.get("positives") != fit.get("negatives")
            or fit.get("feature_dim") != encoder_spec["feature_dim"]
            or fit.get("ridge") != spec["ridge"]
        ):
            return invalid(f"seed {seed} ridge fit audit changed")
        address = row.get("address_audit", {})
        episode_segments = spec["evaluation_episodes"] * (
            spec["candidates_per_episode"] // spec["retrieval"]["address_pool"]
        )
        if (
            address.get("query_topics") != spec["evaluation_episodes"]
            or address.get("candidate_topics")
            != spec["evaluation_episodes"] * spec["candidates_per_episode"]
            or address.get("episode_segment_addresses") != episode_segments
            or address.get("unique_episode_segment_addresses") != episode_segments
            or any(len(address.get(name, "")) != 64 for name in (
                "query_address_sha256", "candidate_address_sha256",
            ))
        ):
            return invalid(f"seed {seed} episode address audit changed")
        score_audit = row.get("score_audit", {})
        if (
            any(len(score_audit.get(name, "")) != 64 for name in (
                "content_scores_sha256", "address_scores_sha256",
            ))
            or score_audit.get("normal_pool_fact_coverage")
            < spec["thresholds"]["minimum_normal_pool_fact_coverage"]
        ):
        ...
        split = arms["balanced_split"]
        topic = arms["topic_only"]
        shuffled_address = arms["shuffled_episode_address"]
        ...
        if (
            ...
            or not thresholds["minimum_topic_only_recall_at_1"]
            <= topic["recall_at_1"]
            <= thresholds["maximum_topic_only_recall_at_1"]
        ):
            return invalid(f"seed {seed} positive, no-memory, or order-leak control failed")
        address_valid = (
            topic["recall_at_3"] >= thresholds["minimum_topic_only_recall_at_3"]
            and shuffled_address["recall_at_3"]
        ...
        verdict = (
            "GRC3A_BALANCED_SPLIT_VALID"
            if address_valid and content_valid
            else "GRC3C_EPISODE_ADDRESS_LOSS"
            if not address_valid
            else "GRC3B_CONTENT_RANKING_LOSS"
        )
    ...
    final = (
        "GRC3A_BALANCED_SPLIT_VALID"
        if all(value == "GRC3A_BALANCED_SPLIT_VALID" for value in verdicts)
        else "GRC3C_EPISODE_ADDRESS_LOSS"
        if any(value == "GRC3C_EPISODE_ADDRESS_LOSS" for value in verdicts)
        else "GRC3B_CONTENT_RANKING_LOSS"
    )
    return {
        ...
        "reason": {
            "GRC3A_BALANCED_SPLIT_VALID": (
                "balanced content ranking and independent episode addressing passed every control"
            ),
            "GRC3B_CONTENT_RANKING_LOSS": (
                "episode addressing passed but balanced content ranking missed a threshold"
            ),
            "GRC3C_EPISODE_ADDRESS_LOSS": (
                "normal or shuffled episode addressing missed a threshold"
            ),
        }[final],
        "spec_sha256": spec_sha256(spec),
        "seeds": summaries,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("measurement/balanced_retrieval_control_adjudication.json"),
    )
