...
case "$ACTION" in
  setup)
    ...
    ;;
  smoke|full|language-preserved|phase-state|phase-state-repair|phase-state-bridge32|state-map)
    cd "$ROOT"
    ...
    ;;
  launch-smoke|launch-full|launch-language-preserved|launch-phase-state|launch-phase-state-repair|launch-phase-state-bridge32|launch-state-map)
    JOB=${ACTION#launch-}
    ...
    ;;
  *)
    echo "usage: $0 setup|smoke|full|language-preserved|phase-state|phase-state-repair|phase-state-bridge32|state-map|launch-* [git-revision]" >&2
    exit 2
    ;;
esac
