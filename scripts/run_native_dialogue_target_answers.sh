...
read -r PRETRAIN_STEPS DIALOGUE_STEPS GLOBAL_BATCH MICRO_BATCH < <("$PYTHON_BIN" -c '
import sys
...
print(s["pretrain_steps"], s["dialogue_steps"], s["global_batch"], s["micro_batch_profiles"][profile])
' "$MICRO_BATCH_PROFILE")
if (( GLOBAL_BATCH % MICRO_BATCH != 0 )); then
  echo "registered global batch must be divisible by target micro batch" >&2
  exit 2
fi
STEPS=$((PRETRAIN_STEPS + DIALOGUE_STEPS))
